...
func _ready():
	...
	cm.add_opt(&"graphics/rendering_method", 0, func(value: int):
		cm.set_project_settings_opt(
				...
				RENDERING_METHODS[value]),
		true,
		["pc"])

	cm.add_opt(&"graphics/window_mode", Window.MODE_WINDOWED, func(value: Window.Mode):
		get_tree().root.set_mode(value))

	cm.add_opt(&"graphics/vsync_mode", DisplayServer.VSYNC_ENABLED, func(value: DisplayServer.VSyncMode):
		DisplayServer.window_set_vsync_mode(value))

	cm.add_opt(&"graphics/ui_scale", 1.0, func(value: float):
		get_tree().root.set_content_scale_size(Vector2i(_viewport_start_size * value)))

	cm.add_opt(&"graphics/fov", 75.0)

	# 3D Scaling
	cm.add_opt(&"graphics/scaling_3d/mode", Viewport.SCALING_3D_MODE_BILINEAR, func(value: Viewport.Scaling3DMode):
		if value == Viewport.SCALING_3D_MODE_FSR or value == Viewport.SCALING_3D_MODE_FSR2:
			cm.set_opt(&"graphics/scaling_3d/scale",
					minf(1, cm.get_opt(&"graphics/scaling_3d/scale")))

		get_viewport().scaling_3d_mode = value)

	cm.add_opt(&"graphics/scaling_3d/fsr_sharpness", 0.2, func(value: float):
		get_viewport().fsr_sharpness = value)

	cm.add_opt(&"graphics/scaling_3d/scale", 1.0, func(value: float):
		if value > 1:
	...
	# Anti Aliasing
	cm.add_opt(&"graphics/msaa3d", Viewport.MSAA_DISABLED, func(value: Viewport.MSAA):
		get_viewport().msaa_3d = value)

	cm.add_opt(&"graphics/taa", false, func(value: bool):
		get_viewport().use_taa = value)

	cm.add_opt(&"graphics/screen_space_aa", Viewport.SCREEN_SPACE_AA_DISABLED, func(value: Viewport.ScreenSpaceAA):
		get_viewport().screen_space_aa = value)

	# Quality
	cm.add_opt(&"graphics/shadows/quality", 3, func(value: int):
	...
	cm.add_opt(&"graphics/shadows/filter", RenderingServer.SHADOW_QUALITY_SOFT_VERY_LOW, func(value: RenderingServer.ShadowQuality):
